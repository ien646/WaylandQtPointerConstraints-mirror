...
LockPointer::LockPointer(QWindow* window)
    : QObject(nullptr) // Never automatically destroy
      , _window(window)
{
    if (_window == nullptr)
    {
    ...
    _registryListener.global =
        [](void* data, wl_registry* registry, const uint32_t name, const char* interface, const uint32_t version)
        {
            ...
            {
                _this->_relativePointerManagerV1 = static_cast<zwp_relative_pointer_manager_v1*>(
                    wl_registry_bind(registry, name, &zwp_relative_pointer_manager_v1_interface, version));
            }
        };
    _registryListener.global_remove = nullptr;

    _relativePointerListenerV1.relative_motion = [](void* data,
                                                    [[maybe_unused]] zwp_relative_pointer_v1* rpointer,
                                                    ...
                                                    const wl_fixed_t delta_x_unaccel,
                                                    const wl_fixed_t delta_y_unaccel)
    {
        auto* _this = static_cast<LockPointer*>(data);
...
LockPointer::~LockPointer()
{
    ...
    if (_relativePointerManagerV1)
    {
        zwp_relative_pointer_manager_v1_destroy(_relativePointerManagerV1);
    }
    if (_relativePointerV1)
    {
        zwp_relative_pointer_v1_destroy(_relativePointerV1);
    }
}
...
void LockPointer::lockPointer(const QRect region)
{
    ...
    auto* waylandSurface = static_cast<wl_surface*>(_nativeInterface->nativeResourceForWindow("surface", _window));
    ...
    _lockedPointerV1 = zwp_pointer_constraints_v1_lock_pointer(
        _pointerConstraintsV1,
        waylandSurface,
        pointer,
        _lockedRegion,
